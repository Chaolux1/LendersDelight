...
public class ImprovedDogFoodItem extends DogFoodItem {
    public ImprovedDogFoodItem(Properties properties) {
        super(properties);
    }

    public static final List<MobEffectInstance> IMPROVED_EFFECTS;

    static {
        IMPROVED_EFFECTS = Lists.newArrayList(new MobEffectInstance[]{new MobEffectInstance(MobEffects.MOVEMENT_SPEED, 12000, 0), new MobEffectInstance(MobEffects.DAMAGE_BOOST, 12000, 0), new MobEffectInstance(MobEffects.DAMAGE_RESISTANCE, 12000, 0)});
    }
        ...
        if (target instanceof LivingEntity entity) {
            if (target.getType().is(ModTags.DOG_FOOD_USERS)) {
                ...
                if (entity.isAlive() && (!isTameable || ((TamableAnimal)entity).isTame()) && itemStack.getItem().equals(ModItems.IMPROVED_DOG_FOOD.get())) {
                    ...
                    for(MobEffectInstance effect : ImprovedDogFoodItem.IMPROVED_EFFECTS) {
